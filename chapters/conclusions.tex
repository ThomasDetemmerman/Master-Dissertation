...
\definecolor{c_otherue}{HTML}{F4A259}
 ...
 \begin{tikzpicture}
  \begin{axis}[
    ...
    symbolic x coords={EIRP Exp Opt, EIRP PwrC Opt, Microstrip Exp Opt, Microstrip PwrC},
    xtick=data,
    nodes near coords, 
  ...
  %myUABS
  \addplot [fill=c_myuabs] coordinates {
({EIRP Exp Opt},50)
({EIRP PwrC Opt},73)
({Microstrip Exp Opt},45)
({Microstrip PwrC},98)};
  %other uabs
  \addplot [fill=c_otheruabs] coordinates {
({EIRP Exp Opt},70)
({EIRP PwrC Opt},105)
({Microstrip Exp Opt},25)
({Microstrip PwrC},30)};
%myue
  \addplot [fill=c_myue] coordinates {
({EIRP Exp Opt},115)
({EIRP PwrC Opt},127)
({Microstrip Exp Opt},85)
({Microstrip PwrC},87)};
%other ue
  \addplot [fill=c_otherue] coordinates {
({EIRP Exp Opt},0)
({EIRP PwrC Opt},0)
