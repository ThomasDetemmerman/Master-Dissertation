...
This scenario investigate the same behavior  as the previous. Still with one drone but for a higher number of users. The scenario can be divided into two cases. One for a variable 
...
Four possible configurations are possible because there are two antennae available (\gls{isotropicradiator} and a realistic antenna) which can both operate in an power consumption optimzed network or an exposure optimized 
network. These four configurations are investigated for the two cases mentioned above.
Both cases can further be divided in four series where 
The $SAR_{10g}$, power consumption and user coverage will be investigated for both cases.
...
\cline{3-4}
                                                        &                               & \multicolumn{2}{c|}{{\color[HTML]{000000} Optimization strategy}} \\ \cline{3-4} 
                                                        &                               & exposure optimized          & power consumption optimized         \\ \hline
\multicolumn{1}{|c|}{}                                  & equivalent istropic radiatior &                             &                                     \\ \cline{2-4} 
\multicolumn{1}{|c|}{\multirow{-2}{*}{\rotatebox[origin=c]{90}{type of antenna}}} & microstrip patch antenna      &                             &                                     \\ \hline
\end{tabular}
\caption{Four possible configurations= (2 optimization strategies + 2 types of antenna) for the decision algorithm}
\end{table}