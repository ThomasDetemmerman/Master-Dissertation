...
Mahfouz et al. performs an analysis on the total \gls{SAR} from a mobile phone considering the fact that the device contains multiple antennas. They
...
far-field and concluded that a difference in \gls{SAR}-values was measured between both scenarios.  In \cite{P7} are the standardization efforts discussed
 ...
 investigate the exposure of a user to a nearby smart meter and concluded that \gls{SAR} was well bellow the guideline restrictions. 
 ...
 exposure from \gls{UABS}s in a \gls{GSM} network is lower than from \gls{UE}. This is however not always the case an \gls{UMTS} network where , thanks to power control,
 the exposure from \gls{UE} is drastically reduced and at some points even equal to radiation from the base station.
...
\section{Summary}
In this chapter are related papers discussed that perform research in the same field as well
as the restrictions and guidelines by different organizations.
First is the legislations about electromagnetic exposure and specific absorption rates discussed followed 
...
has been proposed in the past. Even optimizing towards electromagnetic radiation and power consumption has been investigated. However, 
this one was for terrestrial networks. To the best of the author's knowledge, 
 exposure and power consumption optimized \gls{UAV}-aided networks remains unknown.
