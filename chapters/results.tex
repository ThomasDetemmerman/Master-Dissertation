...
This results in more then 15 million 52 thousand path loss calculations.
...
The power consumption of the entire network is here of course for the only \gls{UABS} available.
...
\begin{figure}[t]
  ...
  \caption{Example of an EIRP \gls{PwrC Opt} network with only one \gls{UABS} (yellow triangle). Only a very few users (green) are connected.
  All other users (red) remain uncovered.}
  \label{fig:s2a:distribution}
\end{figure}

Figure \ref{fig:s2a_dlAndPc}.a presents the total \gls{DL} exposure and the mean whole 
body \gls{SAR} for 20 simulations. The grey area represents the confidence interval. 
...
However, when looking at figure \ref{fig:s2a_dlAndPc}.b, the power consumption in a power consumption optimized network is worse 
...
Eventually, figure \ref{fig:s2shfourSourcesMatrix} shows the total whole body $SAR_{10g}$ deducted from all electromagnetic sources. This combines the exposure 
...
\begin{figure}[!htb]
\subfloat[microstrip, \gls{Exp Opt} ]{\includegraphics[width=0.5\textwidth]{../images/ghentDistribution_expmicro_drones.jpg}}
\subfloat[EIRP, \gls{PwrC Opt}]{\includegraphics[width=0.5\textwidth]{../images/ghentDistribution_pcEIRP_drones.jpg}}
...
Figure \ref{fig:s3a_dlAndPc} shows that the number of required \gls{UAV}s decreases when the flying altitude becomes higher;
