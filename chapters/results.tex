...
Figure \ref{fig:s1_fhsar} shows that the \gls{SAR} of the user originating from the \gls{UABS} remain constant. This means that 
...
also constant. This is normal behaviour since \gls{LTE} makes usage of power control meaning that the antenna won't use more then strictly necessary
to reach the user. The exposure is in other words a constant fraction of power and distance like shown in equation \ref{eq:exposureBasicFormula}.
...
Since the exposure has to remain constant the the distance between the drone and orange user does not increase 
as fast as the blue user, the exposure will increase. Equation \ref{eq:exposureBasicFormula}.
