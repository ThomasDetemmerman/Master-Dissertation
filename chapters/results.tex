...
Not only \gls{isotropicradiator}s are able to cover more users, so do power consumption optimized networks.
 However, this difference becomes more distinguished when applied to larger populations. For example, in a network
...
\gls{UABS}s at a flying altitude of $200\ m$ which increases respectively to 84 and 64 at a much lower flying altitude of $20\ m$.
...
we notice the same behaviour where the 
 \gls{UL} \gls{SAR} indeed increases between 89 and 141 $nW/kg$ when raising the flying altitude from 
 20 to 200 metres.
...
The results shows that by raising the flying altitude from 20 to 200 metres,
the $SAR^{otherUABS}$ will increase between 115 and 140 $nW/kg$ for \gls{EIRP} antenna and between 54 and 74 $nW/kg$ for microstrip patch antennae.
...
When the population increases from 50 to 600 users, a microstrip \gls{Exp Opt} network 
requires 200 more \gls{UABS}s. The number of required \gls{UABS}s in an \gls{EIRP} \gls{Exp Opt} and a microstrip \gls{PwrC Opt} network 
increases with around 130 and the \gls{EIRP} \gls{PwrC Opt} with 110.
...
20 $W$. Once the population is increased to 600 users, a microstrip \gls{Exp Opt} networks will require 130 $W$, 
 a microstrip \gls{PwrC Opt} network requires 117 $W$,
\gls{EIRP} \gls{Exp Opt} networks requires 107 $W$ and \gls{EIRP} \gls{PwrC Opt} networks requires 92 $W$.
...
Figure \ref{fig:s3b_fourSourcesMatrix} shows that the 
\gls{SAR} from the weighted average  user's own device and from the serving \gls{UABS} remains almost constant. The flying altitude is always the same so 
...
The \gls{SAR} values that increases the most is $SAR^{otherUABS}$ which 
starts really low with less than 0.1 $\mu W/kg$ for 50 users for all configurations. 
The \gls{SAR} increases however very fast. The biggest increase in noticed in an \gls{EIRP} \gls{PwrC Opt} network 
...
The last scenario shows that less \gls{UABS} are required for higher flying altitudes and therefore 
result in a decreased power consumption.  Exposure from the serving \gls{UABS} remain almost constant above 80 meters while 
exposure from other \gls{UABS}s and \gls{UE} increases. Scenario III also shows that more \gls{UABS} are 
required for larger populations and therefore increases power consumption and electromagnetic exposure.
Finally, the results also confirms that the optimization algorithm works as intended 
