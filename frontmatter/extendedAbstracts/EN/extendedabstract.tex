...
The  results show that for a varying flying height, a logarithmic relationship exist between the $P_{tx}$ and the flying height. 
...
Each time the flying height becomes to large to cover, the 
$P_{tx}$ increases with one dBm. 
When using the default configuration where the maximum allowed $P_{tx}$ is set to 33 dBm,
a \gls{UABS} can fly up to 387 meters before losing connection in a free \gls{LOS} scenario.
...
A power consumption optimized network has the highest exposure which is behaviour that was already proven by \cite{J1}. 
However, power consumption optimized network will have higher power consumption then in an exposure optimized network. 
...
\gls{UABS} fly higher, the exposure increases and more users become covered and . 
The increasing electromagnetic radiation is however not unlimited.
At even higher
flying altitudes, the distance between a given \gls{UABS} and users further away becomes to large causing the 
coverage to decrease again. When this decrease occurs depends on the configuration. An power consumption optimized 
...
The contribution to the total \gls{SAR} from each individual source is identical to the previous scenario. \textcolorr{red}{ << deze zin weg?}

There is still only one \gls{UABS} is available. So when the population grows, more users become uncovered and 
therefore decreasing the average electromagnetic exposure.
...
since there are a lot of obsturcting buildings at this flying altitude.
