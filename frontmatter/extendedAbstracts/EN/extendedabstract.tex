...
The sufficiently large aperture angle covers enough users. 
The antenna is recommended to be deployed in a power consumption 
...
The second subsection gives an overview of how a microstrip patch antenna can be 
designed.
...
\begin{figure}[h!]
  ...
  \caption{These figures show how various sizes of population influence the downlink electromagnetic radiation of the average user (fig. a) and 
  total power consumption (fig. b).}
...
\begin{figure}[h!]
  ...
  \caption{These two figures show how the flying height influences the downlink electromagnetic radiation of the average user (fig. a) and 
  power consumption of the entire network (fig. b) for an unlimited number of \gls{UAV}s.}
...
A first consequence of raising the flying altitude from 
 20 to 200 metres is an increase in \gls{SAR} originating the user's own device and is situated between 89 and 141 $nW/kg$; a behaviour also explained in the first scenario.
...
\begin{figure}[h!]
\centering
  \includegraphics[width=0.96\linewidth]{../results/s3/uFourSources.png}
...
\begin{figure}[hb!]
\centering
  \includegraphics[width=0.9\linewidth]{../images/fourCasesMatrixSol.png}
...
\centering
 \resizebox {!} {3.5cm} {
\begin{tikzpicture}[nodes = {scale=2}]
  \foreach \percent/\name in {
      52/ my UE,
      33/ other UABSs,
      15/ my UABS,
...
power consumption of $114\ V/m$, which is $1.19\ W$ per \gls{UABS}.

As a future work, still some parameters require further evaluation. Different \acs{pusch} values
